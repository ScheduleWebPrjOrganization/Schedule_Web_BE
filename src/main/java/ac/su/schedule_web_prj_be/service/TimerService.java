...
//    }
    ...
    public List<SubjectRecord> getCurrentStudyingSubjects() {
        return subjectRecordRepository.findAll().stream()
                .filter(record -> record.getStoppedAt() == null /*&& record.getPausedAt() == null*/)  // 일시정지 기능 주석처리
    ...
    public long getTotalStudyTimeForSubject(Subject subject) {
        return subjectRecordRepository.findAll().stream()
                ...
                .mapToLong(record -> {
                    ...
                    return (endTime - record.getRecordedAt().getTime()) /*- record.getPausedDuration()*/;  // 일시정지 기능 주석처리
                })
                .sum();
    }
